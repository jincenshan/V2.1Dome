  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column class-name="status-col" label="大气压压力" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.outputPressure }}</span></template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button style="padding: 0px;" type="text" size="small" @click="historyShow(scope.row)" v-if="buttonArray.includes('network_hs_n')">历史记录</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    methods: {
      ...
      exportExcel() {
        var params = {
          ...
          excelColumn: ['id', 'controlSn', 'netName', 'modelName', 'terminal','totalOxyOutput','useTotalDuration','continuousUseDuration', 'potency', 'minPotency','maxPotency', 'outputPressure', 'altitude', 'humidity','co2Density','status','runningState','activationDate','modifyDate'],
          excelHead: ["终端ID", "终端SN", "所驻网点", "终端设备型号", "房间内取氧口数量","累计出氧量(L)","总运行时长（h）", "连续运行时长（min）", "浓度(%)", "浓度下限","浓度上限","大气压压力", "相对海拔（m）", "湿度", "二氧化碳浓度PPM", "设备使用状态","设备运行状态","激活时间","最近一次更新时间"],
          export: true,
        }
