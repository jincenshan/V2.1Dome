<template>
  <el-tabs type="border-card" v-model="pane_name" @tab-click="computerList()" style="margin-top: 50px">
    <el-tab-pane label="制氧主机维护" name="on">
      <div>
        <el-button type="danger" @click="addShow(1)" style="margin-bottom: 20px" v-if="buttonArray.includes('room_add_oxy')">添加制氧机</el-button>
        <el-button type="danger" @click="runShow()" style="margin-bottom: 20px;margin-left: 20px"
                   v-if="buttonArray.includes('run_date_n') && list_table_data.length != 0 && !is_type">运行参数设置
        </el-button>
      </div>
      <el-table
...
<!--            <span>{{ scope.row.modelName }}</span>-->
      ...
      <el-dialog title="机房运行参数设置" :visible.sync="is_distribution" :close-on-click-modal="false" width="1000px">
        <el-form :model="distribution_form">
          <div style="margin-bottom: 10px">吹洗设置</div>
          <el-table
            v-loading="listLoading"
            :data="list_table"
            element-loading-text="Loading"
          ...
          >
            ...
            <el-table-column label="均压时间（秒）" width="150" align="center">
              <template slot-scope="scope">
                <el-input v-model="scope.row.params.avgPressureTime" v-if="scope.row.params" class="input_class" v-input-filter:int/>
              </template>
            </el-table-column>
            <el-table-column label="吹洗时间（秒）" width="150" align="center">
              <template slot-scope="scope">
                <el-input v-model="scope.row.params.purgingTime" v-if="scope.row.params" class="input_class" v-input-filter:int/>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="created_at" label="压力设置（Mpa）" width="200">
              <template slot-scope="scope">
                <div style="justify-content: center;align-items: center;display: flex;flex-direction: row">
                  <el-input v-if="scope.row.params" v-model="scope.row.params.pressureSettingInitial" class="input_class" style="width: 50px" v-input-filter:one/>
                  <el-input v-if="scope.row.params" v-model="scope.row.params.pressureSettingEnd" class="input_class" style="width: 50px" v-input-filter:one/>
                </div>
              </template>
            </el-table-column>
            <el-table-column align="center" prop="created_at" label="空压机延时设置（秒）" width="200">
              <template slot-scope="scope">
                <el-input v-if="scope.row.params" v-model="scope.row.params.compressorDelayedSetting" class="input_class" placeholder="请输入"
                          v-input-filter:int/>
              </template>
            </el-table-column>
          </el-table>
        </el-form>
        <div slot="footer" class="dialog-footer" style="display: flex;flex-direction: row;align-items: center;justify-content: space-between">
          <span style="width: 70%;text-align:left;color: #8c939d;line-height:16px;font-size: 12px">设备压力值不能设置成一样；一般压力值越小的，在供氧时会优先启动供氧，吹洗时间，设置在90秒以内</span>
          <div>
            <el-button @click="closeShow()">取 消</el-button>
            <el-button type="primary" @click="submitShow()" style="margin-left: 20px">确 定</el-button>
          </div>
        </div>
      </el-dialog>
...
flex-direction: column;width:280px;margin-top: 20px;margin-left: 20px">
    ...
    <el-tab-pane label="配套部件" name="up" v-if="!is_type">
        ...
        <div>
          ...
          >
            <el-table-column
              ...
              align="center"
            >
              <template slot-scope="scope">第{{ scope.row.id}}号制氧主机</template>
            </el-table-column>
          </el-table>
        </div>
        <div style="margin-left: 10px">
          <div
            style="display: flex;flex-direction: row;justify-content: space-around;padding:10px 0px;background-color: #d3dce6;align-items: center">
            <label><a>第{{groupId}}号制氧主机</a></label>
            <el-button type="danger" @click="addPart()" v-if="buttonArray.includes('room_oxy_add_part_n')">新增</el-button>
          </div>
          <el-table
            v-loading="listLoading"
          ...
          >
            ...
            <el-table-column class-name="status-col" label="使用状态" width="110" align="center">
              <template slot-scope="scope" v-if="scope.row.status =='NORMAL'">正常使用</template>
              <template slot-scope="scope" v-else>停止使用</template>
            </el-table-column>
          </el-table>
        </div>
      </div>
      <el-dialog title="部件规格添加" :visible.sync="is_part" :close-on-click-modal="false" width="1000px" @close="colseClickOn()">
        <div class="tab_main_div">
          <div class="tab_main_hread">
            <el-button type="danger" style="height: 40px" @click="addShowPart(1)">新增</el-button>
            <a>SN:{{groupSN}}&nbsp;&nbsp;设备规格维护</a>
            <a></a>
          </div>
          <div class="tab_mian_content">
            <div style="display: flex;flex-direction: column;">
              <el-button class="tab_button" v-if="!list">请先关联部件</el-button>
              <el-button class="tab_button" v-if="list" v-for="item in list" :key="item.relId" :style='partId == item.partId?"background-color:#97a8be":"background-color:aliceblue"'
                         @click="getRelShow({partId:item.partId,relId:item.relId,partName:item.partName})">{{item.partName}}
              </el-button>
            </div>
            <el-table
            ...
            >
              ...
              <el-table-column align="center" prop="created_at" label="请输入S/N" width="450">
                <template slot-scope="scope" style="display: flex;flex-direction: row;justify-content: space-between;align-items: center">
                  <div style="display: flex;flex-direction: row;align-items: center;justify-content: center">
                    <el-input v-model="scope.row.snNo" :disabled="is_input" class="input_class" placeholder="请输入S/N" clearable
                              style="width: 150px;border: none" v-input-filter:china/>
                    <el-button type="primary"  v-if='!is_input && user.type == "OPT"' @click="addShow({id:scope.row.id,snNo:scope.row.snNo})"
                               style="width: 70px;height: 30px;padding:0px">提交
                    </el-button>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                  <el-button type="text" size="small" v-if='is_input && user.type == "OPT"' @click="editShow({id:scope.row.id,specId:scope.row.specId})">更换</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
      ...
      <el-dialog :title="'SN:'+groupSN+'选择部件规格'" :visible.sync="is_part_from" width="500px" :close-on-click-modal="false">
        <div class="tab_mian_content" style="background-color: white;position: relative;z-index: 9999">
          <div v-if="!is_edit">
            <el-button class="tab_button" v-for="item in list_button" :key="item.partId"
                       @click="buttonShow(item.partId)">{{item.partName}}
            </el-button>
          </div>
          <div>
            <el-radio-group v-model="specId" style="display: flex;
flex-direction: column;width:280px;margin-top: 20px;margin-left: 20px">
        ...
        <div style="display: flex;flex-direction: row;justify-content: flex-end;align-items: center;margin-top: 20px">
          <span v-if="list_button_spec.length == 0" style="margin-right: 20px;font-size: 10px;color: #97a8be">暂无更多可替换规格，请到设备部件管理配置</span>
          <el-button type="primary" @click="addShowPart(2)" style="width: 70px;height: 30px;padding:0px"
                     v-if="!is_edit">确定
          </el-button>
          <el-button type="primary" @click="updateShow()" style="width: 70px;height: 30px;padding:0px"
                     v-if="is_edit">确定
          </el-button>
        </div>
      </el-dialog>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.is_type = this.$router.currentRoute.query.is_type == 'FISSION' ? true : false
    ...
    methods: {
      ...
      submitShow() {
        ...
        for (let i in this.list_table) {
          let param = this.list_table[i];
          param.params.equipId = param.id
          param.params.controlId = this.controlId
          if((4.5 > param.params.pressureSettingInitial ||  param.params.pressureSettingInitial > 5.5 ) || (4.5 > param.params.pressureSettingEnd ||  param.params.pressureSettingEnd > 5.5 )){
            this.$message.error("压力值，需在4.5和5.5之间")
            return;
          }
          if(param.params.pressureSettingInitial>param.params.pressureSettingEnd){
            this.$message.error("起始压力值大于结束压力值")
            return;
          }
          for (let [key,value] of Object.entries(param.params)){
            if(value == ""){
              this.$message.error("请填写完整信息")
              return;
            }
          }
          paramsArr.push(param.params);
        }
        data_params.params = paramsArr;
        equipSetting(getBaseUrl(data_params)).then(res => {
          if(res.code !=0) {
            // this.$router.go(0)
      ...
      getGroupListOxy(row) {
        if (row) {
          this.groupId = row.id
          this.groupSN = row.snNo
        }
        equipmentPartSpecList(getBaseUrl({equipId: this.groupId})).then(res => {
          if(res.code !=0) {
      ...
      addShowPart(e) {
        ...
        if (e == 1) {
          ...
          equipmentSpecPartNo(getBaseUrl({equipId: this.groupId, typeCode: "OXY"})).then(res => {
            if(res.code !=0) {
      ...
      editShow(e) {
        ...
        // // this.is_part_from = true
        // this.notInId = e.id
        // equipmentPartDetail(getBaseUrl({equipId: this.groupId,partId:this.partId,notInId:e.specId})).then(res =>{
        //   if(res.code !=0) {
        //     this.list_button_spec = res.data
        //   }
        // })
