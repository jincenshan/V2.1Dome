<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="username" class="input_class" placeholder="按账户名称" clearable/>
      <el-input v-model="registerSource" class="input_class" placeholder="按注册来源" clearable/>
      <el-input v-model="area" class="input_class" placeholder="按所在地区" clearable/>
      <el-date-picker
        v-model="value1"
        type="daterange"
        range-separator="至"
        start-placeholder="按注册日期"
        end-placeholder="按注册日期"
      ...
      <el-date-picker
        ...
        type="daterange"
        range-separator="至"
        start-placeholder="最近一次登录"
        end-placeholder="最近一次登录"
        style="margin-left: 10px"
      />
    ...
    >
      <el-table-column align="center" label="用户编号" width="200">
      ...
      <el-table-column label="社会职务" width="150" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="注册来源" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.registerSource }}</span>
        </template>
      </el-table-column>
      <el-table-column label="所在地区" width="150" align="center">
        <template slot-scope="scope">{{ scope.row.area }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="订单数" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.orderNumber }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="氧疗袋数量" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.bagNumber }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="剩余赠送次数" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.remainingGifts }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="注册时间" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.createDate }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="最近一次登录时间" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.modifyDate }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="邀请人" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.invitaionAccount }}</template>
      </el-table-column>
  ...
  methods: {
    ...
    fetchData() {
      var params = {
        ...
        beginCreateDate:this.value2[0] ? parseTime(this.value2[0]).substring(0, 10) + ' 00:00:00' : '',
        endCreateDate:this.value2[1] ? parseTime(this.value2[1]).substring(0, 10) + ' 23:59:59' : '',
    ...
    exportExcel() {
      var params = {
        ...
        excelColumn: ['id', 'username', 'name', 'profession', 'registerSource', 'area', 'orderNumber','bagNumber','remainingGifts','createDate','modifyDate','invitaionAccount'],
        excelHead: ["用户编号", "账号", "昵称", "社会职务", "注册来源", "所在地区", "订单数","氧疗袋数量","剩余赠送次数","注册时间","最近一次登录时间","邀请人"],
        export: true,
      }
    ...
    //账户明细
    detailShow(row) {
      this.$router.push({name: 'dot_detail', query: {accountId: row.id}})
    }
  }
}
</script>
