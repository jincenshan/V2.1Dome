<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-select v-model="type" clearable placeholder="订单状态" style="width: 150px">
        <el-option
      ...
      <el-date-picker
        ...
        type="daterange"
        range-separator="至"
        start-placeholder="按更新时间"
        end-placeholder="按更新时间"
    ...
    >
      ...
      <el-table-column label="建行商户号" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.mktMrchId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="创建时间" width="150" align="center">
        <template slot-scope="scope">{{ scope.row.createDate }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="最近一次更新时间" width="150" align="center">
        <template slot-scope="scope">{{ scope.row.modifyDate }}</template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="100px">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="detailShow(scope.row)">账户明细</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {accountBobList, goodsSpec, princeCityQuery} from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      accountBobList(params).then(res => {
        if (res.code != 0) {
    ...
    exportExcel() {
      var params = {
        ...
        excelColumn: ['id', 'name', 'type', 'balance', 'mktMrchId', 'createDate', 'modifyDate'],
        excelHead: ["账户编号", "账户名称", "账户类型", "当前累计分润金额", "建行商户号", "创建时间", "最近一次更新时间"],
        export: true,
      }
