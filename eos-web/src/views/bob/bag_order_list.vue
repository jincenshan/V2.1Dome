  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if='scope.row.status =="已退款" && buttonArray.includes("bob_refund")' @click="handleClick(scope.row,1)">退款详情
          </el-button>
          <el-button type="text" size="small" v-if='(scope.row.status =="已付款" || scope.row.status =="已供氧")&& buttonArray.includes("bob_refund_in") '
                     @click="handleClick(scope.row,2)">退款
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="订单退款" :visible.sync="is_from" width="450px" @close="colseClick" :close-on-click-modal="false">
      <el-form :model="ruleForm" ref="ruleForm" label-width="70px" class="demo-ruleForm">
        ...
        <el-form-item label="退款原因">
          ...
          <span v-if="ruleForm.type == 1">{{ruleForm.remark}}</span>
        </el-form-item>
        <el-form-item label="退款时间" v-if="ruleForm.type == 1">
          <span>{{ ruleForm.claimDate }}</span>
        </el-form-item>
        <el-form-item label="操作人" v-if="ruleForm.type == 1">
          <span>{{ ruleForm.updatedName }}</span>
        </el-form-item>
        <div style="display: flex;flex-direction: row;justify-content: space-around;align-items: center" v-if="ruleForm.type == 1">
          <span>退款后，一般采用原路返回</span>
          <el-button type="primary" @click="submitForm()" style="width: 100px">确认退款</el-button>
        </div>
      </el-form>
    </el-dialog>
...
<script>
import {bbyOrderList, goodsSpec, princeCityQuery,refundDetail,refundOrder} from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    colseClick() {
      ...
      this.ruleForm.claimDate = ""
      this.ruleForm.updatedName = ""
    },
    handleClick(row, type) {
      this.is_from = true
      this.ruleForm.type = type
      if (type == 1) {
        refundDetail({orderId: row.id}).then(res => {
          if(res.code != 0) {
            this.ruleForm.orderId = res.data.orderId
            this.ruleForm.amount = res.data.amount
            this.ruleForm.channel = res.data.channel
            this.ruleForm.remark = res.data.remark
            this.ruleForm.claimDate = res.data.claimDate
            this.ruleForm.updatedName = res.data.updatedName
          }
        })
      } else {
        this.ruleForm.orderId = row.id
        this.ruleForm.amount = row.amount
        this.ruleForm.channel = row.channel
      }
    },
    submitForm() {
      refundOrder({orderId: this.ruleForm.orderId,remark: this.ruleForm.remark}).then(res =>{
        if(res.code != 0){
