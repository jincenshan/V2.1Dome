<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="snNo" class="input_class" placeholder="按科室氧疗终端S/N" clearable/>
    ...
    >
      ...
      <el-table-column class-name="status-col" label="终端SN" width="150" align="center">
...
<script>
import {controlMonitorList, homeShutdown, equipmentList, deptflowList} from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    historyShow(row){
      this.$router.push({name: 'department_history', query: {equipId: row.id, typeCode: "CONTROL",snNo:row.snNo}})
    },
    searchShow() {
      this.id = ""
      this.fetchData()
    ...
    exportExcel() {
      var params = {
        ...
        excelColumn: ['id', 'netName', 'snNo', 'deptName', 'modelName', 'totalOxyOutput', 'monthlyDosage', 'oxygenPressure', 'airPressure','negativePressure', 'instantFlow', 'status', 'createTime'],
        excelHead: ["终端ID", "网点名称", "终端SN", "科室名称", "设备型号", "累计用氧量m³", "本月用氧量", "氧气压力Kpa","压缩空气压力Kpa","负压压力传感器",  "瞬时流量(L/min)", "设备使用状态", "最近一次更新时间"],
        export: true,
      }
