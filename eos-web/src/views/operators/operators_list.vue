<template>
  <div class="app-container">
    ...
    <div class="block" style="justify-content: flex-end">
      ...
      <el-button type="danger" @click="AddOperators()" v-if="buttonArray.includes('add_operator_n')">新增</el-button>
    ...
    >
      ...
      <el-table-column label="所在地区" width="200" align="center">
      ...
      <el-table-column class-name="status-col" label="代理产品线" width="150" align="center">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="150">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="handleClick(scope.row)" v-if="buttonArray.includes('detail_operator_n')">运营商详情</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        operatorList(getBaseUrl(list_params)).then(res => {
          if(res.code != 0) {
      ...
      handleClick(row) {
        if (this.user.type == "PLATFORM") {
          this.$router.push({name: 'operators_details', query: {id: row.id, type: 1,bby:row.productLine}})
        } else if(this.user.type == "OPT") {
          this.$router.push({name: 'operators_details', query: {id: row.id, type: 2,bby:row.productLine}})
        }else {
          this.$router.push({name: 'operators_details', query: {id: row.id, type: 3,bby:row.productLine}})
        }
      },
      go_dot_show(e){
        this.$router.push({name: 'operators_dot_list',query: {optId:e}})
      },
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['id','name','area','contacts','phone','netCount','type','productLine','createDate'],
          excelHead:["运营商编号","运营商名称","所在地区","联系人","联系人电话","已入驻网点数","运营商类型","代理产品线","创建时间"],
          export:true,
        }
        ...
        operatorList(getBaseUrl(params)).then(res =>{
          if(res.code != 0) {
