<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-button type="danger" @click="AddOperators()" v-if="buttonArray.includes('add_oxy_operator_n')">新增</el-button>
    ...
    >
      <el-table-column align="center" label="运营商编号" width="180">
      ...
      <el-table-column class-name="status-col" label="已入驻网点数" width="150" align="center">
        <template slot-scope="scope">
          <span style="color: #20a0ff" @click="go_dot_show(scope.row.id)" v-if="scope.row.netCount !=0">
            {{ scope.row.netCount }}
          </span>
      ...
      <el-table-column class-name="status-col" label="运营商类型" width="150" align="center">
        <template slot-scope="scope">{{scope.row.tps.join(",")}}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="200px">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="buttonArray.includes('add_oxy_detail_n')" @click="handleClick(scope.row)">运营商详情</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      go_dot_show(e){
        console.log(e)
        this.$router.push({name: 'operator_on_dot',query:{optId:e}})
      },
      handNextClick(){
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['id','name','area','contacts','phone','netCount','tps','createDate'],
