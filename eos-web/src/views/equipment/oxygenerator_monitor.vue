<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="所属运营商" width="150" align="center">
        <template slot-scope="scope">{{ scope.row.optName }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="所属控制器 " width="150" align="center">
        <template slot-scope="scope">{{ scope.row.controlName }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="制氧机识别编号" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.distCode }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="制氧机型号" width="110" align="center">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="300" v-if="buttonArray.includes('oxy_address_n')||buttonArray.includes('oxy_status_n')||buttonArray.includes('oxy_part_n')||buttonArray.includes('oxy_addres_n')">
        <template slot-scope="scope">
          ...
          <el-button type="text" style="padding: 0px;" size="small" @click="handleClick(scope.row)" v-if="buttonArray.includes('oxy_part_n')">部件运行详情</el-button>
          <el-button type="text" style="padding: 0px;" size="small" @click="addressShow(scope.row)" v-if="buttonArray.includes('oxy_hs_n')">历史记录</el-button>
    ...
    <el-dialog title="制氧机使用状态维护" :visible.sync="is_from" width="400px":close-on-click-modal="false">
    ...
    methods: {
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['id','snNo','optName','roomName','controlName','productLine','distCode','modelName','totalOxyOutput','useTotalDuration','outputPressure','outputPressure','potency','instantFlow','status','runningState','activationDate','lastSyncTime'],
          excelHead:["制氧机ID","制氧机SN","所属运营商","所属机房","所属控制器","产品线","制氧机识别编号","制氧机型号","累计出氧量(m³)","总运行时长(h)","输出压力(kg/m²)","出气压力(kg/m²)","氧气浓度(%)","瞬时流量(L/min)","设备使用状态","设备运行状态","激活时间","最近一次更新时间"],
          export:true,
        }
