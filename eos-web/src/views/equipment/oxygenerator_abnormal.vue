<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="snNo" class="input_class" placeholder="请输入制氧机S/N" clearable />
      <el-input v-model="roomName" class="input_class" placeholder="请输入所属机房" clearable/>
      <el-select v-model="modelId" clearable placeholder="按制氧机型号" style="width: 160px">
        <el-option
      ...
      <el-select v-model="equipStatus" clearable placeholder="按使用状态" style="margin-left: 10px;width: 160px">
        <el-option
      ...
      <el-select v-model="exStatus" clearable placeholder="问题解决状态" style="margin-left: 10px;width: 160px">
        <el-option
    ...
    >
      ...
      <el-table-column label="制氧机识别编号" width="120" align="center">
        <template slot-scope="scope">{{ scope.row.distCode }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="设备型号" width="110" align="center">
      ...
      <el-table-column class-name="status-col" label="报警问题类型" width="160" align="center">
<!--        <template slot="header" slot-scope="scope">-->
<!--          <span>报警问题类型</span>-->
<!--          <el-tooltip class="item" effect="dark" content="如果超过一小时，仍请求不到，则整机会显示为整机-报警" placement="right-end">-->
<!--            <i class="el-icon-question" style="width: 20px;height: 20px;margin-left: 15px"></i>-->
<!--          </el-tooltip>-->
<!--        </template>-->
        <template slot-scope="scope">{{ scope.row.info }}
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="设备使用状态" width="110" align="center">
        <template  slot-scope="scope">
          <span v-if="scope.row.useStatus == 'NORMAL'">正常使用</span>
          <span  v-if="scope.row.useStatus == 'STOP'">停止使用</span>
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" label="是否解决" width="110" align="center">
        <template slot-scope="scope">{{ scope.row.exStatus }}</template>
      </el-table-column>
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="buttonArray.includes('oxy_ok') && scope.row.problemStatus !='已解决'" @click="clickOk(scope.row)">解决</el-button>
<!--          <el-button type="text" size="small" v-if="buttonArray.includes('oxy_ab_detail')" @click="handleClick(scope.row)">部件运行详情</el-button>-->
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import {controlExceptionList, equipmentList, oxyExceptionList, oxyExceptionSolve} from '@/api/table'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        oxyExceptionList(list_params).then(res => {
          if(res.code != 0) {
      ...
      handleClick(row) {
        this.$router.push({name: 'part_sun_details', query: {equipId: row.id, typeCode: "OXY",snNo:row.equipSn}})
      },
      listDetail(e){
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['id','equipSn','roomName','distCode','modelName','productLine','totalOxyOutput','info','createDate','reason','solvePlan','useStatus','exStatus'],
          excelHead:["异常报警ID","设备SN","所属机房","制氧机识别编号","设备型号","产品线","累计出氧量(L)","报警问题类型","发生时间","原因","解决方案","设备使用状态","是否解决"],
          export:true,
        }
        ...
        oxyExceptionList(params).then(res =>{
          if(res.code != 0) {
