<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="controlSn" class="input_class" placeholder="请输入控制器S/N" clearable />
      ...
      <el-select v-model="status" clearable placeholder="按使用状态" style="margin-left: 20px;width: 160px">
        <el-option
      ...
      <el-select v-model="productLine" clearable placeholder="请选择产品线" style="margin-left: 20px;width: 160px">
        <el-option
    ...
    >
      ...
      <el-table-column label="管理制氧机台数" width="120" align="center" >
        <template slot-scope="scope">
          <span style="color: #20a0ff" @click="oxyDetail(scope.row)" v-if="scope.row.oxyCount">{{ scope.row.oxyCount }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="250" v-if="buttonArray.includes('controller_hs_n') || buttonArray.includes('status_n') || buttonArray.includes('part_status_n')">
        <template slot-scope="scope">
          <el-button type="text" style="padding: 0px;" size="small" @click="clickOk(scope.row)" v-if="buttonArray.includes('c_status_n')">设备重启</el-button>
          <el-button type="text" style="padding: 0px;" size="small" @click="handleClick(scope.row)" v-if="buttonArray.includes('part_status_n')">部件运行详情</el-button>
          <el-button type="text" style="padding: 0px;" size="small" @click="addressShow(scope.row)" v-if="buttonArray.includes('controller_hs_n')">历史记录</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="控制器使用状态维护" :visible.sync="is_from" :close-on-click-modal="false" width="400px">
...
<script>
  import {equipmentList, controlMonitorList, controlShutdown, controlExceptionList, activationOk} from '@/api/table'
    ...
    methods: {
      fetchData() {
        let list_params = {
          ...
          startTime: this.timedata[0] ? parseTime(this.timedata[0]).substring(0, 10) + ' 00:00:00' : '',
          ...
          controlSn: this.controlSn,
          productLine: this.productLine,
          token: sessionStorage.getItem("token"),
        }
        controlMonitorList(list_params).then(res => {
          if(res.code != 0) {
      ...
      // changeShow(column, prop, order ){
      //   this.proptype = prop; // 将键名prop赋值给变量proptype
      //   if (column.order === "descending") {
      //     this.list.sort(this.my_desc_sort);
      //   } else if (column.order === "ascending") {
      //     this.list.sort(this.my_asc_sort);
      //   }
      // },
      // //排序方法
      // my_desc_sort(a, b) {
      ...
      handleClick(row) {
        this.$router.push({name: 'part_sun_details', query: {equipId: row.id, typeCode: "CONTROL",snNo:row.snNo}})
      },
      //激活
      clickOk(row){
        ...
        this.$confirm('设备重启', '您确定要重启设备?', {
          ...
          controlShutdown(add_data).then(res=>{
            if(res.code != 0) {
              this.$router.go(0)
            }
          })
      ...
      submitForm(){
        ...
        controlShutdown(getBaseUrl(add_data)).then(res=>{
          if(res.code != 0) {
            this.is_from = false
            this.$router.go(0)
          }
      ...
      addressShow(row){
        this.$router.push({name: 'controller_history', query: {equipId: row.id, typeCode: "CONTROL",snNo:row.snNo}})
      },
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['id','snNo','roomName','modelName','productLine','distCode','oxyCount','totalOxyOutput','useTotalDuration','continuousUseDuration','oxyTankLowPressure','oxyTankHighPressure','instantFlow','status','runningState','activationDate','lastSyncTime'],
          excelHead:["控制器ID","控制器SN","所驻机房","控制器型号","所属产品线","网点识别编号","管理制氧机台数","累计出氧量(L)","总运行时长(H)","连续运行时长（min）","源氧罐压力(kpa)","供氧罐压力(kpa)","瞬时流量(L/min)","设备使用状态","设备运行状态","激活时间","最近一次更新时间"],
          export:true,
        }
        ...
        controlMonitorList(params).then(res =>{
          if(res.code != 0) {
