<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="snNo" class="input_class" placeholder="按控制终端S/N" clearable/>
      <el-input v-model="optName" class="input_class" placeholder="按所属运营商" clearable />
      <el-select v-model="modelId" clearable placeholder="按终端型号">
        <el-option
    ...
    <div class="block" style="justify-content: flex-end">
      ...
      <el-button type="danger"  @click="addShow()" v-if="user.type == 'PLATFORM'">新增</el-button>
      <button type="button" class="el-button el-button--primary el-button--medium" @click="dialogFormVisible = true" v-if="list.length !=0 && buttonArray.includes('canExport')"><i class="el-icon-document" /><span>
        导出 Excel
      </span></button>
    </div>
    <el-table
    ...
    >
      <el-table-column align="center" label="终端设备S/N" width="150">
      ...
      <el-table-column label="终端设备型号" width="150" align="center">
      ...
      <el-table-column class-name="status-col" label="当前所在网点" width="150" align="center">
        <template slot-scope="scope">{{ scope.row.netName }}</template>
      </el-table-column>
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="!scope.row.optName && user.type == 'PLATFORM'" @click="addShow(scope.row)">销售出库</el-button>
<!--          <el-button type="text" size="small" v-if="!scope.row.optName && user.type == 'PLATFORM'" @click="handleClick(scope.row)">关联设备规格</el-button>-->
          <el-button type="text" size="small" v-if="scope.row.optName" @click="go_monitor(scope.row)">设备运行监控</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="is_out?'销售出库':'新增取氧终端入库记录'" :visible.sync="is_from" @close="colseClick()" width="500px" :close-on-click-modal="false">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
          <el-form-item label="设备SN:" v-if="is_out" prop="name">
            <span>{{ruleForm.snNOOne}}</span>
          </el-form-item>
          <el-form-item label="取氧终端设备S/N" prop="snNo" style="margin-top: 20px" v-if="!is_out">
            <el-input v-model="ruleForm.snNo" style="width: 250px" maxlength="50" v-input-filter:china></el-input>
          </el-form-item>
          <el-form-item label="设备名称" v-if="!is_out" prop="equipTypeCode">
            <el-select v-model="ruleForm.equipTypeCode" clearable placeholder="请选择设备名称"
                       @change="selectShow(ruleForm.equipTypeCode)" style="width: 250px;">
              <el-option
          ...
          <el-form-item label="终端型号" v-if="!is_out" prop="modelIds">
            <el-select v-model="ruleForm.modelIds" clearable placeholder="请选择终端型号" style="width: 250px;">
              <el-option
          ...
          <el-form-item label="SIM卡ICCID 1" prop="iccidOne" v-if="!is_out">
            <el-input v-model="ruleForm.iccidOne" style="width: 250px"></el-input>
          </el-form-item>
          <el-form-item label="SIM卡ICCID 2" v-if="!is_out">
            <el-input v-model="ruleForm.idccidTwo" style="width: 250px"></el-input>
          </el-form-item>
          ...
          <div style="display: flex;flex-direction: row;justify-content: center;align-items: center">
            <span style="font-size: 12px;color: #5a5e66;margin-right: 20px" v-if="is_out">提交后不可修改，请谨慎选择</span>
            <span style="font-size: 12px;color: #5a5e66;margin-right: 20px" v-if="!is_out">首次保存后，后续只能编辑SIM卡ICCID</span>
...
<script>
  import {equipmentList, equipmentSpecPartList,equipmentStorageIn,operatorRoleSelect,equipmentStorageOut} from '@/api/table'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          equipTypeCode:[{required: true,message:"请输入", trigger: 'blur' }],
          iccidOne:[{required: true,message:"请输入", trigger: 'blur' }],
    ...
    created() {
      ...
      equipmentList(getBaseUrl({
        ...
        name: "",
        pageSize: 100,
        typeCode: 'OXY_UPTAKE,GENERAL_CONTROL,TRANSFER,ENTER_HOUSE,SUB_CONTROL,OVERALL_SUB_CONTROL,DEPT_TERMINAL'
      })).then(res => {
        if(res.code !=0) {
          let arr = []
    ...
    methods: {
      ...
      handleClick(row) {
        this.$router.push({name: 'associated_device', query: {id: row.id,typeCode:"OXY_UPTAKE",snNo:row.snNo}})
      },
      go_monitor(row){
        this.$router.push({name: 'out_up_oxygenerator', query: {id: row.id,typeCode:"OXY_UPTAKE"}})
      },
      ...
      exportExcel() {
        var params ={
          ...
          excelColumn: ['snNo','name','modelName','createDate','person','isActivation','activationDate','optName','netName'],
          excelHead:["终端设备S/N","设备名称","终端设备型号","入库时间","入库人","激活状态","激活时间","当前所属运营商","当前所属网点"],
          export:true,
        }
      ...
      submitForm(e){
        if(e == 1){
          ...
          if(this.ruleForm.modelIds == '' || this.ruleForm.snNo == '' || this.ruleForm.operator == '' || this.ruleForm.iccidOne == '' || this.ruleForm.equipTypeCode == ''){
            this.$message.error("请填写完整")
            return
          }

          let add_params = {
            iccidOne: this.ruleForm.iccidOne,
            idccidTwo: this.ruleForm.idccidTwo,
            modelId: this.ruleForm.modelIds,
            modelName: 0,
            name: "",
            operator: this.ruleForm.operator,
            snNo: this.ruleForm.snNo,
            typeCode: this.ruleForm.equipTypeCode
          }
          if(this.ruleForm.equipTypeCode =="" ||this.ruleForm.snNo == "" || this.ruleForm.operator==""||this.ruleForm.modelIds == "" ||this.ruleForm.iccidOne == ""){
