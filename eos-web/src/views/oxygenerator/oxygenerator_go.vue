<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="snNo" class="input_class" placeholder="按制氧机S/N" clearable />
      <el-input v-model="optName" class="input_class" placeholder="按所属运营商" clearable />
      <el-select v-model="modelId" clearable placeholder="按制氧机型号">
        <el-option
...
<!--      <el-select v-model="type_state" clearable placeholder="按使用状态" style="margin-left: 10px">-->
<!--        <el-option-->
...
<!--      </el-select>-->
    ...
    <div class="block" style="justify-content: flex-end">
      ...
      <el-button type="danger"  @click="addShow()" v-if="buttonArray.includes('add_oxy')">新增</el-button>
    ...
    >
      <el-table-column align="center" label="制氧机设备S/N" width="150">
        <template slot-scope="scope">
          {{scope.row.snNo}}
        </template>
      </el-table-column>
      <el-table-column label="制氧机型号" width="150" align="center">
      ...
      <el-table-column label="操作" align="center" width="250">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="!scope.row.optName && buttonArray.includes('oxy_up')" @click="addShow(scope.row)">销售出库</el-button>
<!--          <el-button type="text" size="small" v-if=" !scope.row.optName && buttonArray.includes('oxy_type')" @click="handleClick(scope.row)">关联设备规格</el-button>-->
          <el-button type="text" size="small" v-if=" scope.row.optName && buttonArray.includes('oxy_m')" @click="go_monitor(scope.row)">设备运行监控</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="is_out?'销售出库':'新增制氧机入库记录'" :visible.sync="is_from" @close="colseClick()" width="500px" :close-on-click-modal="false">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="130px" class="demo-ruleForm">
          <el-form-item label="设备SN:" v-if="is_out" prop="name">
            <span>{{ruleForm.snNOOne}}</span>
          </el-form-item>
          <el-form-item label="制氧机设备S/N" prop="snNo" style="margin-top: 20px" v-if="!is_out">
            <el-input v-model="ruleForm.snNo" maxlength="50" style="width: 250px" v-input-filter:china></el-input>
          </el-form-item>
          <el-form-item label="制氧机型号" v-if="!is_out" prop="modelId">
            <el-select v-model="ruleForm.modelId" clearable placeholder="按制氧机型号" style="width: 250px">
              <el-option
                v-for="item in options_ids"
                :key="item.id"
                :label="item.name"
...
<script>
  import {equipmentList, equipmentSpecPartList,equipmentStorageIn,operatorRoleSelect,equipmentStorageOut,equipmentSpecSelect } from '@/api/table'
  ...
  export default {
    ...
    created() {
      this.fetchData()
      equipmentSpecSelect(getBaseUrl({typeCode:"OXY"})).then(res=>{
        this.options_spec = res.data
      })
    ...
    methods: {
      ...
      handleClick(row) {
        this.$router.push({name: 'associated_device', query: {id: row.id,typeCode:"OXY",snNo:row.snNo}})
      },
      go_monitor(row){
      ...
      exportExcel() {
        var params ={
          ...
          excelHead:["制氧机设备S/N","制氧机型号","入库时间","入库人","激活状态","激活时间","当前所属运营商"],
          export:true,
        }
