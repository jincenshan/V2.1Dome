<template>
  <div class="app-container">
    <div class="block">
      <el-input v-model="snNo" class="input_class" placeholder="请输入控制终端S/N" clearable/>
      <el-input v-model="netName" class="input_class" placeholder="请输入所属网点" clearable/>
      <el-select v-model="modelId" clearable placeholder="按房间终端型号" style="width: 150px">
        <el-option
    ...
    <div class="block" style="display:flex;margin-top:20px;flex-direction:row;justify-content:flex-end;margin-bottom: 20px">
    ...
    >
      ...
      <el-table-column class-name="status-col" label="房间内取氧口数量" width="150" align="center">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button style="padding: 0px;" type="text" size="small" @click="clickOk(scope.row)">使用状态管理</el-button>
          ...
          <el-button style="padding: 0px;" type="text" size="small" @click="historyShow(scope.row)" v-if="buttonArray.includes('home_hs_n')">历史记录</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import {controlMonitorList, homeShutdown, equipmentList, terminalList} from '@/api/table'
  ...
  export default {
    ...
    created() {
      ...
      equipmentList(getBaseUrl({
        ...
        name: "",
        pageSize: 100,
        typeCode: 'OVERALL_SUB_CONTROL,SUB_CONTROL'
      })).then(res => {
        if (res.code != 0) {
          let arr = []
    ...
    methods: {
      ...
      historyShow(row){
        this.$router.push({name: 'home_history', query: {equipId: row.id,snNo:row.snNo}})
      },
      handleClick(row) {
        this.$router.push({name: 'part_sun_details', query: {equipId: row.id, typeCode: "HOME",snNo:row.snNo}})
      },
      searchShow() {
        this.id = ""
        this.fetchData()
      ...
      exportExcel() {
        var params = {
          ...
          excelColumn: ['id', 'snNo', 'netName', 'modelName', 'terminal', 'totalOxyOutput','useTotalDuration', 'continuousUseDuration', 'potency','minPotency','maxPotency', 'atmosphericPressure', 'altitude','temperature','humidity', 'co2Density', 'status','runningState','activationDate','lastSyncTime'],
          excelHead: ["终端ID", "终端SN", "所驻网点", "终端设备型号", "房间内取氧口数量", "累计出氧量(L)","总运行时长（h）", "连续运行时长（min）", "浓度(%)","浓度下限", "浓度上限","大气压压力", "相对海拔（m）", "温度", "湿度","二氧化碳浓度PPM","设备使用状态","设备运行状态","激活时间", "最近一次更新时间"],
          export: true,
        }
